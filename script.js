...
const firebaseConfig = {
  ...
  appId: "1:813586117280:web:bb1db15b33c8b85cb94fc5"
};
...
const soruSirasi = document.getElementById('soru-sirasi');
const puanGoster = document.getElementById('puan');
const sureGoster = document.getElementById('sure');
const soruGoster = document.getElementById('soru');
const cevapButonlari = document.getElementById('cevapbutonlari');
const toplamPuanGoster = document.getElementById('toplam-puan');
const liderlerListesi = document.getElementById('liderler');
const loggedInUserId = localStorage.getItem('loggedInUserId');

// Oyun Başlatma
document.getElementById('oyna-btn').addEventListener('click', () => {
...
document.getElementById('tekrar-oyna-btn').addEventListener('click', () => {
    sonucEkrani.style.display = 'none';
    oyunEkrani.style.display = 'block';
    mevcutSoruIndex = 0;
    mevcutPuan = 0;
    sure = 60;
    puanGoster.innerText = `Puan: ${mevcutPuan}`;
    soruSirasi.innerText = `Soru: 1`;
    zamanlayici = setInterval(zamanlayiciGuncelle, 1000);
    sonrakiSoru();
});

document.getElementById('lt-btn').addEventListener('click', liderlikTablosuGoster);
document.getElementById('geri-btn').addEventListener('click', () => {
    liderlikTablosu.style.display = 'none';
    baslangicEkrani.style.display = 'block';
});

function sonrakiSoru() {
    if (mevcutSoruIndex >= sorular.length || sure <= 0) {
        oyunBitti();
        return;
    }

    soruSirasi.innerText = `Soru: ${mevcutSoruIndex + 1}`;
    puanGoster.innerText = `Puan: ${mevcutPuan}`;
    sureGoster.innerText = `Süre: ${sure}`;

    const mevcutSoru = sorular[mevcutSoruIndex];
    soruGoster.innerText = mevcutSoru.soru;

    while (cevapButonlari.firstChild) {
        cevapButonlari.removeChild(cevapButonlari.firstChild);
    }

    mevcutSoru.cevaplar.forEach(cevap => {
        const buton = document.createElement('button');
        buton.innerText = cevap;
        buton.classList.add('btn');
        if (cevap === mevcutSoru.dogruCevap) {
            buton.dataset.dogru = true;
        }
        buton.addEventListener('click', cevapSecildi);
        cevapButonlari.appendChild(buton);
    });
}

function cevapSecildi(e) {
    const secilenButon = e.target;
    const dogru = secilenButon.dataset.dogru;
    ...
    Array.from(cevapButonlari.children).forEach(buton => {
        buton.removeEventListener('click', cevapSecildi);
        if (buton.dataset.dogru) {
...
function oyunBitti() {
    ...
    toplamPuanGoster.innerText = mevcutPuan;

    db.collection('liderlikTablosu').add({
        kullanici: loggedInUserId,
        puan: mevcutPuan,
        zaman: firebase.firestore.FieldValue.serverTimestamp()
    });
}

function liderlikTablosuGoster() {
    liderlerListesi.innerHTML = '';
    baslangicEkrani.style.display = 'none';
    liderlikTablosu.style.display = 'block';

    db.collection('liderlikTablosu').orderBy('puan', 'desc').limit(10).get().then(snapshot => {
        snapshot.forEach(doc => {
            const lider = doc.data();
            const liderElem = document.createElement('div');
            liderElem.innerText = `${lider.kullanici} - ${lider.puan} puan`;
            liderlerListesi.appendChild(liderElem);
        });
    });
}