...
import { getFirestore, collection, getDocs } from "https://gstatic.com/firebasejs/10.12.2/firebase-firestore.js";
...
const firebaseConfig = {
    ...
    appId: "1:813586117280:web:79f09071c1064b54b94fc5"
};
...
document.getElementById("lt-btn").addEventListener("click", async () => {
    const querySnapshot = await getDocs(collection(db, "users"));
    let leaderboard = [];
    querySnapshot.forEach((doc) => {
        let data = doc.data();
        leaderboard.push({ username: data.username, score: data.bilgiyarismasi });
    });
...
function displayLeaderboard(leaderboard) {
    let leaderboardHTML = "<h2>Liderlik Tablosu</h2><ul>";
    leaderboard.forEach(user => {
        leaderboardHTML += `<li>${user.username}: ${user.score}</li>`;
    });
    leaderboardHTML += "</ul>";

    document.getElementById("baslangic-ekrani").innerHTML = leaderboardHTML;
}