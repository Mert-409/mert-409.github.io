...
async function puanGuncelleFirestore(kullaniciId, yeniPuan) {
    ...
    const yarismaRef = doc(kullaniciRef, "bilgiyarismasi", "puan");

    try {
        const yarismaSnap = await getDoc(yarismaRef);
        if (yarismaSnap.exists()) {
            const mevcutPuan = yarismaSnap.data().puan;
            if (yeniPuan > mevcutPuan) {
                await updateDoc(yarismaRef, { puan: yeniPuan });
                console.log("Puan güncellendi.");
            } else {
                console.log("Mevcut puan daha yüksek.");
            }
        } else {
            await setDoc(yarismaRef, { puan: yeniPuan });
...
async function liderlikTablosuGoster() {
    ...
    const q = query(collection(db, "users"), orderBy("bilgiyarismasi.puan", "desc"), limit(10));
    const querySnapshot = await getDocs(q);
    querySnapshot.forEach(doc => {
        const data = doc.data();
        liderlikTablosuEkrani.innerHTML += `<li>${data.users}: ${data.bilgiyarismasi.puan}</li>`;
    });
