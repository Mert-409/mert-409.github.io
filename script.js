...
import { getFirestore, doc, getDoc, setDoc, updateDoc, collection, getDocs, query, orderBy, limit } from "https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js";
...
const liderlikTablosuEkrani = document.getElementById('liderlik-tablosu');
...
document.getElementById("hesap-btn").addEventListener("click", function() {
    window.location.href = "https://mert-409.github.io/accounts/";
});
...
function oyunuBaslat() {
    ...
    liderlikTablosuEkrani.style.display = 'none';
    ...
    butonlar.forEach(buton => {
        ...
        if (buton.textContent === seciliCevap) {
            if (seciliCevap === dogruCevap) {
                ...
                setTimeout(() => {
                    buton.classList.remove('dogru');
                    butonlar.forEach(buton => buton.disabled = false);
                }, 500);
                puan += 100;
            } else {
                buton.classList.add('yanlis');
                setTimeout(() => {
                    buton.classList.remove('yanlis');
                    butonlar.forEach(buton => buton.disabled = false); 
                }, 500);
                sure -= 2;
            }
...
function oyunuBitir() {
    ...
    const kullaniciId = localStorage.getItem('loggedInUserId');
    if (kullaniciId) {
        puanGuncelleFirestore(kullaniciId, puan);
    }
}

async function puanGuncelleFirestore(kullaniciId, yeniPuan) {
    const kullaniciRef = doc(db, "users", kullaniciId);

    try {
        const yarismaSnap = await getDoc(kullaniciRef);
        if (yarismaSnap.exists()) {
            const mevcutPuan = yarismaSnap.data().bilgiyarismasi || 0;
            if (yeniPuan > mevcutPuan) {
                await updateDoc(kullaniciRef, { 'bilgiyarismasi': yeniPuan });
                console.log("Puan güncellendi.");
            } else {
                console.log("Mevcut puan daha yüksek.");
            }
        } else {
            await setDoc(kullaniciRef, { 'bilgiyarismasi': yeniPuan });
            console.log("Yeni puan kaydedildi.");
        }
    } catch (error) {
...
async function liderlikTablosuGoster() {
    baslangicEkrani.style.display = 'none';
    oyunEkrani.style.display = 'none';
    sonucEkrani.style.display = 'none';
    liderlikTablosuEkrani.style.display = 'block';
    liderlikTablosuEkrani.innerHTML = '<h2>Liderlik Tablosu</h2><ul>';

    const q = query(collection(db, "users"), orderBy("bilgiyarismasi", "desc"), limit(50));
    const querySnapshot = await getDocs(q);
    let sıra = 1;
    querySnapshot.forEach(doc => {
        const data = doc.data();
        liderlikTablosuEkrani.innerHTML += `<li>${sıra}. ${data.username}: ${data.bilgiyarismasi}</li>`;
        sıra++;
    });

    liderlikTablosuEkrani.innerHTML += '</ul>';
}
